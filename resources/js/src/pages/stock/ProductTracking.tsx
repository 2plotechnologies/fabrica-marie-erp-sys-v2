...
import { Search, Package, Truck, ShoppingCart, ArrowRightLeft, PackageX } from 'lucide-react';
...
const StockVendedoresPage = () => {
  ...
  const filtered = stockVendedores.filter(sv => {
    ...
    const matchesVendedor =
      filterVendedor === 'all' ||
      String(sv.vendedor_id) === filterVendedor ||
      String(sv.vendedor?.id) === filterVendedor ||
      String(sv.vendedor?.usuario_id) === filterVendedor ||
      String(sv.vendedor?.usuario?.id) === filterVendedor;
  ...
  const totalAsignado = filtered.reduce((s, sv) => s + Number(sv.cantidad_entregada || 0), 0);
  const totalVendido = filtered.reduce((s, sv) => s + Number(sv.vendido || 0), 0);
  const totalDefectuosos = filtered.reduce((s, sv) => s + Number(sv.defectuosos || 0), 0);
  const totalDisponible = filtered.reduce((s, sv) => s + Number(sv.cantidad_entregada || 0) - Number(sv.vendido || 0) - Number(sv.devuelto || 0), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 animate-slide-up">
        ...
        <Card className="shadow-card"><CardContent className="pt-6"><div className="flex items-center gap-4"><div className="h-12 w-12 rounded-xl bg-red-100 dark:bg-red-900/30 flex items-center justify-center"><PackageX className="h-6 w-6 text-red-600" /></div><div><p className="text-sm text-muted-foreground">Defectuosos</p><p className="text-2xl font-bold text-red-600">{totalDefectuosos}</p></div></div></CardContent></Card>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        {!isVendedor && (
          <Select value={filterVendedor} onValueChange={setFilterVendedor}><SelectTrigger className="w-[220px]"><SelectValue placeholder="Todos los vendedores" /></SelectTrigger><SelectContent><SelectItem value="all">Todos los vendedores</SelectItem>{vendedores.map(v => <SelectItem key={v.id} value={String(v.id)}>{v.usuario?.nombre}</SelectItem>)}</SelectContent></Select>
        )}
      </div>

      <div className="bg-card rounded-xl border shadow-card overflow-hidden">
        <Table>
          <TableHeader><TableRow><TableHead>Vendedor</TableHead><TableHead>Producto</TableHead><TableHead>Fecha</TableHead><TableHead className="text-right">Asignado</TableHead><TableHead className="text-right">Vendido</TableHead><TableHead className="text-right">Devuelto</TableHead><TableHead className="text-right text-red-500 font-semibold">Defectuosos</TableHead><TableHead className="text-right">Disponible</TableHead></TableRow></TableHeader>
          <TableBody>
            {paginatedRecords.map(sv => (
              <TableRow key={sv.id}>
                ...
                <TableCell className="text-right text-red-600 font-semibold">{Number(sv.defectuosos || 0)}</TableCell>
                <TableCell className="text-right"><Badge variant={Number(sv.cantidad) > 0 ? 'default' : 'secondary'}>{Number(sv.cantidad)}</Badge></TableCell>
              </TableRow>
            ))}
            {filtered.length === 0 && <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">No hay stock asignado a vendedores. Crea una Salida de Fábrica y despacha.</TableCell></TableRow>}
          </TableBody>
        </Table>
        {totalPages > 1 && (
